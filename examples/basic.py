...
# (see setup_example_db() below), where the join has a unique key (user_id). We
# want to store the result of the join in a fast storage.
...
async def main():
    ...
    storage_table = tcl.LocalStorageTable(indexes)
    ...
    async with db_access, asyncpg.create_pool(POSTGRES_DSN) as postgres_pool:
        ...
        # load in a background task, we could have awaited CachedTable.loaded()
        # to wait until it's ready (e.g. in a readiness check). Since we're
        ...
        async for user in table.get_records('primary_key', 1, 2):
            ...
            # (the arguments are againg turned into an IndexSpec which we could
        ...
        # pass an IndexSpec that gets the record we've updated twice (each in a
        # list with one element). After we've invalidated record, the cache
        # makes sure that the next time we query that primary key, it is
        # fetched from the DB first.
        await postgres_pool.execute(
            'UPDATE users SET name = $1 WHERE user_id = $2', 'New Name', 1)
        print('User 1 still with old name:')
        print(await table.get_first_record('primary_key', 1))
        table.invalidate_records(
            [indexes.IndexSpec('primary_key', 1)],
            [indexes.IndexSpec('primary_key', 1)])
        print('User 1 with new name:')
        print(await table.get_first_record('primary_key', 1))
        # We can also invalidate records that were added to the DB after the
        # cache was loaded, which will cause them to be added to storage.
        await postgres_pool.execute('''
