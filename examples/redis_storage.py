...
# TODO doc not recommended, use local instead+++++++++++++

# TODO include await table.loaded()++++++++
# TODO switch to native storage, but provide a redis example+++++++++
# TODO example with getting multiple+++++++++++
# TODO example with recheck predicate++++++++++
# TODO example with multiple indexes where one is meant for adjusting and one
# for accessing (i.e. with UnsupportedIndexOpertation)+++++++++++++++
# TODO example with adjusting, explain that long adjusts don't block read
# operations and provide a consistent view of the old version+++++++++++++
# TODO example with composite primary key++++++++++++
...
async def main():
    ...
    db_access = tc.PostgresTable(postgres_pool)
    ...
    indexes = tc.PrimaryKeyIndexes(
        'user_id', query_all_string, query_some_string)
    primary_key_name = 'user_id'
    storage_table = tcr.RedisTable(
        redis_conn, table_name='users_cities',
        primary_key_name=primary_key_name,
        attribute_codecs={
            'user_id': tcr.IntAsStringCodec(),
    ...
    table = tc.CachedTable(
        indexes, db_access, storage_table, primary_key_name=primary_key_name)
    async with contextlib.AsyncExitStack() as stack:
        ...
        # TODO no, this doesn't work anymore+++++++++++++++++
